...
class Order:
    ...
    def get_combos(self):
        """
        Get combo, for instance the higher price of burger, side and drink that user has chosen
        If user don't choose any burger or drink or side, then number_of_combos = 1
        """
        number_of_combos = None

        for category_item in self.items_by_category.values():
            if number_of_combos is None:
                number_of_combos = len(category_item)
            else:
                number_of_combos = min(number_of_combos, len(category_item))

        # sort items by their price, from lowest to highest
    ...
    def __str__(self):
        ...
        for category in self.items_by_category.values():
            for item in category:
                ...
                price = item['price']
                subcategory = item['subcategory'] if item['subcategory'] is not None else ""
